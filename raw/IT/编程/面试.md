...
* 编码: python2默认ascii，python3默认UTF8
* 字符串：Python3使用str表示字符串，bytes表示字节
* 
...
bitmap 最大位数为2**32-1：因为sds限制512m
...
* 先删缓存，后更数据库：有可能数据不一致，可以更新数据库后再加一次删缓存
* 先更数据库，后删缓存：数据不一致可能性较小，但主从库情况下仍有可能，同样可以考虑延迟双删
* 操作可能失败：加入重试。消息队列或者其他中间件
...
* 单一职责
  * 引起类变化的因素应当尽可能少
* 里氏替换
  * 父类出现的地方可以用子类代替，并且保证原来程序的逻辑行为（behavior）不变及正确性不被破坏
  * 子类出现的地方不一定可以用父类代替
* 依赖倒转
...
* 接口隔离
  * 避免接口的过度臃肿
  * 如果调用者只依赖中接口中的一部分方法，那么这个接口的隔离性是不足的：接口提供给调用者的应当都是调用者需要使用的方法。
* 迪米特（最少知识原则）
...
[为什么Python 3.6以后字典有序并且效率更高？](https://www.kingname.info/2019/07/13/python-dict/)

[Python中Dict两种实现的原理](https://juejin.cn/post/6982229357989199880)

[Python 2 和 Python 3 有哪些主要区别？](https://www.zhihu.com/question/19698598)

[一文搞懂高频面试题之限流算法，从算法原理到实现，再到对比分析](https://juejin.cn/post/6870396751178629127#heading-0)
